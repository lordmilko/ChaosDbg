...
namespace ChaosDbg.SymStore
{
    ...
    public class HttpSymbolStore : SymbolStore
    {
        ...
        /// </summary>
        /// <param name="logger">The tracer to use for logging progress messages</param>
        ...
        public HttpSymbolStore(ISymStoreLogger logger, SymbolStore backingStore, Uri symbolServerUri, string personalAccessToken = null) : base(logger, backingStore)
        {
        ...
        protected async Task<Stream> GetFileStream(string path, Uri requestUri, CancellationToken token)
        {
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    {
                        Logger.Error("Not Found: {0} - '{1}'", fileName, requestUri.AbsoluteUri);
                        break;
                    }
                ...
                Logger.Information($"HttpSymbolStore: retry #{retries}");
